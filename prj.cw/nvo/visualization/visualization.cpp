...
vtkSmartPointer<vtkActor> Visualization::generateSphereWithPoints(const std::vector<std::vector<double>>& points, double opacity, double color[3], double radius, double sphereColor[3]) const {
    // Create sphere
    vtkSmartPointer<vtkActor> sphereActor = createSphere(radius, sphereColor, opacity);
...
vtkSmartPointer<vtkActor> Visualization::createSphere(double radius, double color[3], double opacity) const {
...
vtkSmartPointer<vtkActor> Visualization::Mapping(const std::string& imagePath, const std::string& stlPath) const {
    // Создаем сферу с помощью функции createSphere
    vtkSmartPointer<vtkActor> sphereActor = createSphere(1.0, nullptr, 1.0);
    ...
    sphereActor->GetMapper()->SetInputConnection(sphere->GetOutputPort());
